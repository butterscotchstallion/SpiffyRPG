...
class SpiffyBattle:
    ...
    def add_battle(self, **kwargs):
        ...
        units died.
        """
        combatant_1 = kwargs["combatant_1"]
        combatant_2 = kwargs["combatant_2"]
        total_rounds = kwargs["rounds"]

        if combatant_1.id not in self.battles and \
        combatant_2.id not in self.battles:
            battle_info = {
            ...
            self.battles[combatant_1.id] = battle_info
    ...
    def get_last_battle_by_combatants(self, **kwargs):
        """
        Two possibilities: the iniator of the battle
        has a key in battles, or the combatant is 
        the opponent in one of the battles
        """
        combatant = kwargs["combatant"]

        if combatant.id in self.battles:
            return self.battles[combatant.id]

        for battle in self.battles:
            if battle["combatant"].id == combatant:
                return battle
    ...
    def start(self):
        ...
        if attack_hit_target:
            ...
            if last_battle is None:
                log.error("SpiffyRPG: attack hit target no but battle with %s" % target.name)
                return
            ...
            if not hit_info["is_draw"]:
                last_battle = target.get_last_battle_by_combatant(combatant=attacker)

                if last_battle is None:
                    log.error("SpiffyRPG: target riposte but no battle for attacker %s" % target.name)
                    return
        ...
        if not attacker.is_alive():
            ...
            last_battle = winner.get_last_battle_by_combatant(combatant=attacker)
        ...
        if not target.is_alive():
            ...
            last_battle = winner.get_last_battle_by_combatant(combatant=target)
        ...
        if battle_completed:
            ...
            """
            Struck units determines who gets loot, so this
            should be reset after each battle.
            """
            winner.reset_struck_units()
            loser.reset_struck_units()
    ...
    def on_battle_completed(self, **kwargs):
        ...
        if hit_info["is_draw"]:
            ...
            """
            Distribute loot from loser of battle
            """
            self.distribute_loot([loser])
    ...
    def distribute_loot(self, losers):
        ...
        for loser in losers:
            ...
            """
            if len(loser.units_that_have_struck_me) > 0 and len(loser.items) > 0:
                """
                Any unit that has struck this unit gets a random inventory
                item from that unit, if it's not something they already have.
                """
                for struck_unit in loser.units_that_have_struck_me:
                    """
                    Randomize lootable item for each unit
                    """
                    loot_item = loser.get_random_lootable_item(already_has=struck_unit.items)

                    if loot_item is None:
                        log.info("SpiffyRPG: %s has no lootable items that this unit does not already have" % loser.get_name())
                        continue
    ...
    def add_party_member(self, unit):
        if unit not in self.party:
            if unit.is_alive():
                log.info("SpiffyRPG: adding %s to party" % unit.get_name())
...
class SpiffyBattleAnnouncer(SpiffyAnnouncer):
    ...
    def units_found_loot(self, **kwargs):
        ...
        loot = kwargs["item"]
        unit_names = [self._b(unit.get_name()) for unit in units]
        unit_len = len(unit_names)
        params = (self._b(dead_unit.get_name()), self._b(loot.name))
        announcement_msg = ""
        ...
        announcement_msg += " %s in their weakened state and finds %s!" % params
    ...
    def hot_streak_ended(self, **kwargs):
        ...
        ended_by_name = self._b(ended_by.get_name())
    ...
    def unit_dialogue(self, unit, dialogue):
        ...
        """
        if dialogue is None:
            log.error("SpiffyWorld: dialogue error for unit %s" % unit.get_name())
            return
    ...
    def unit_death(self, **kwargs):
        ...
        slain_by_name = self._b(slain_by.get_name())

        name = self._b(unit.get_name())
    ...
    def unit_victory(self, **kwargs):
        ...
        new_params = (winner_title, rounds_won, total_rounds, loser_title)
        announcement_msg = u"%s won %s/%s rounds against %s" % new_params

        if winner.is_player and kwargs["xp_gained"] > 0:
            announcement_msg += " %s gained %s %s" % (winner_title, green_xp, internet_points)
...
class SpiffyPlayerAnnouncer(SpiffyAnnouncer):
    ...
    def found_loot(self, **kwargs):
        ...
        item = kwargs["item"]
        unit_name = self._b(slain_unit.get_name())
        item_name = self._b(item.name)
    ...
    def inventory(self, **kwargs):
        player = kwargs["player"]
        irc = kwargs["irc"]
        items = sorted(player.items, key=lambda x: x.item_type, reverse=True)
            ...
            if unit is not None:
                ...
                if equip_ok:
                    ...
                    if can_battle is True:
                        ...
                        """
                        if not unit.is_player:
                            unit.equip_random_weapon(avoid_weapon_type="scissors")
            ...
            if unit is not None:
                ...
                if equip_ok:
                    ...
                    if can_battle is True:
                        ...
                        """
                        if not unit.is_player:
                            unit.equip_random_weapon(avoid_weapon_type="lizard")
    ...
    def flevel(self, irc, msg, args, level):
        ...
        if dungeon is not None:
            ...
            if unit is not None:
                ...
                log.info("SpiffyRPG: setting xp for %s to %s (level %s)" % (unit.get_name(), xp_for_level, int_level))
    ...
    def doQuit(self, irc, msg):
        ...
        if user_id is not None:
            ...
            if dungeon is not None:
                ...
                if unit is not None:
                    log.info("SpiffyRPG: removing %s from dungeon" % unit.get_name())
                ...
                if dungeon is not None:
                    ...
                    if player is not None:
                        ...
                        if last_welcome > self.welcome_message_cooldown_in_seconds:
                            dungeon.announcer.unit_info(unit=player, dungeon=dungeon, irc=irc)
