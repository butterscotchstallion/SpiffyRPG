...
class SpiffyDungeonAnnouncer(SpiffyAnnouncer):
    ...
    def player_info(self, **kwargs):
        ...
        xp_marginal_requirement = xp_req_for_this_level - xp_req_for_previous_level
        
        """ If this is one, they're max level """
        if xp_req_for_this_level == 1:
            xp_req_for_this_level = self.levels[-1][1]

        percent_xp = round((((float(player_xp) / xp_marginal_requirement) / (float(xp_req_for_this_level) / xp_marginal_requirement)) * 100), 1)
